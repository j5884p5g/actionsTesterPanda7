...
func TestParseCopyrightLine(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			line:    "// Copyright IBM Corp. 2024, 2023",
			lineNum: 1,
			expectedInfo: &CopyrightInfo{
				LineNumber:   1,
				OriginalLine: "// Copyright IBM Corp. 2024, 2023",
				Holder:       "IBM Corp.",
				StartYear:    2023,
				EndYear:      2023,
				Prefix:       "// ",
		...
		},
		{
			...
			line:    "// Copyright IBM Corp. 2024, 2025",
			lineNum: 1,
			expectedInfo: &CopyrightInfo{
				LineNumber:   1,
				OriginalLine: "// Copyright IBM Corp. 2024, 2025",
				Holder:       "IBM Corp.",
				StartYear:    2022,
				EndYear:      2025,
				Prefix:       "// ",
		...
		},
		{
			...
			line:    "/* Copyright IBM Corp. 2024, 2023 - All rights reserved */",
			lineNum: 1,
			expectedInfo: &CopyrightInfo{
				LineNumber:   1,
				OriginalLine: "/* Copyright IBM Corp. 2024, 2023 - All rights reserved */",
				Holder:       "IBM Corp.",
				StartYear:    2023,
				EndYear:      2023,
				Prefix:       "/* ",
		...
		},
		{
			...
			line:    "// Copyright IBM Corp.", 2024, 2026
			lineNum: 1,
			expectedInfo: &CopyrightInfo{
				LineNumber:   1,
				OriginalLine: "// Copyright IBM Corp.", 2024, 2026
				Holder:       "IBM Corp.",
				StartYear:    0,
				EndYear:      0,
				Prefix:       "// ",
...
func TestExtractAllCopyrightInfo(t *testing.T) {
	...
	fileContent := `// Copyright IBM Corp. 2024, 2023
...
func TestUpdateCopyrightHeader(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			initialContent: `// Copyright IBM Corp. 2024, 2023
package main
`,
			...
			expectModified:   true,
			expectedContent: `// Copyright IBM Corp. 2024, 1000+48)) + string(rune((currentYear/100)%10+48)) + string(rune((currentYear/10)%10+48)) + string(rune(currentYear%10+48)) + `
package main
`,
		},
		{
...
`,
			...
			expectedContent: `// Copyright IBM Corp. 2024, 2023
package main
`,
		},
		{
			name: "No update needed",
			initialContent: `// Copyright IBM Corp. ` + string(rune(currentYear/ 2024, 1000+48)) + string(rune((currentYear/100)%10+48)) + string(rune((currentYear/10)%10+48)) + string(rune(currentYear%10+48)) + `
package main
`,
			targetHolder:   "IBM Corp.",
...
`,
		...
		},
		{
			...
			fileContent: `// Copyright IBM Corp. ` + string(rune(currentYear/ 2024, 1000+48)) + string(rune((currentYear/100)%10+48)) + string(rune((currentYear/10)%10+48)) + string(rune(currentYear%10+48)) + `
package main
`,
			targetHolder:      "IBM Corp.",
			configYear:        currentYear,
...
func TestUpdateCopyrightHeader_SkipCopywriteConfig(t *testing.T) {
	...
	fileContent := `// Copyright IBM Corp. 2024, 2020
schema_version = 1
`
...
`,
		...
		{
			name: "Regular file",
			content: `// Copyright IBM Corp. 2024, 2023
package main
...
func TestParseCopyrightLine_StrictCopyrightCheck(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			line:      "// Copyright IBM Corp. 2024, 2020",
			expectNil: false,
		},
		{
...
func TestParseCopyrightLine_InlineComment(t *testing.T) {
	line := "var x := 1 // Copyright IBM Corp. 2024, 2023"
...
func TestUpdateCopyrightHeader_InlineCommentPreserved(t *testing.T) {
	...
	initial := "var x := 1 // Copyright IBM Corp. 2024, 2023\n"
	...
	content, err := os.ReadFile(testFile)
	require.NoError(t, err)
	expected := "var x := 1 // Copyright IBM Corp. 2024, 2022, " + strconv.Itoa(currentYear) + "\n"
...
func TestUpdateCopyrightHeader_IgnoreYear1(t *testing.T) {
	...
	initial := `// Copyright IBM Corp. 2024, 2023
package main
`
	err := os.WriteFile(testFile, []byte(initial), 0644)
	require.NoError(t, err)
...
func TestUpdateCopyrightHeader_IgnoreYear1_NoYearsInHeader(t *testing.T) {
	...
	initial := `// Copyright IBM Corp. 2024, 2026
package main
`
	err := os.WriteFile(testFile, []byte(initial), 0644)
	require.NoError(t, err)
	...
	content, err := os.ReadFile(testFile)
	require.NoError(t, err)
	expected := fmt.Sprintf("// Copyright IBM Corp. 2024, 2021, %d\npackage main\n", currentYear)
	assert.Equal(t, expected, string(content))
}
