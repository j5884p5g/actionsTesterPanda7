...
func TestParseCopyrightLine(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			line:    "// Copyright IBM Corp. 2026",
			lineNum: 1,
			expectedInfo: &CopyrightInfo{
				LineNumber:   1,
				OriginalLine: "// Copyright IBM Corp. 2026",
				Holder:       "IBM Corp.",
				StartYear:    2023,
				EndYear:      2023,
				Prefix:       "// ",
		...
		},
		{
			...
			line:    "/* Copyright IBM Corp. 2026 - All rights reserved */",
			lineNum: 1,
			expectedInfo: &CopyrightInfo{
				LineNumber:   1,
				OriginalLine: "/* Copyright IBM Corp. 2026 - All rights reserved */",
				Holder:       "IBM Corp.",
				StartYear:    2023,
				EndYear:      2023,
				Prefix:       "/* ",
...
func TestExtractAllCopyrightInfo(t *testing.T) {
	...
	fileContent := `// Copyright IBM Corp. 2026
...
func TestUpdateCopyrightHeader(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			initialContent: `// Copyright IBM Corp. 2026
package main
`,
			...
			expectModified:   true,
			expectedContent: `// Copyright IBM Corp. 2026+48)) + string(rune((currentYear/100)%10+48)) + string(rune((currentYear/10)%10+48)) + string(rune(currentYear%10+48)) + `
package main
`,
		},
		{
...
`,
			...
			expectedContent: `// Copyright IBM Corp. 2026
package main
`,
		},
		{
			name: "No update needed",
			initialContent: `// Copyright IBM Corp. ` + string(rune(currentYear/ 2026+48)) + string(rune((currentYear/100)%10+48)) + string(rune((currentYear/10)%10+48)) + string(rune(currentYear%10+48)) + `
package main
`,
			targetHolder:   "IBM Corp.",
...
`,
		...
		},
		{
			...
			fileContent: `// Copyright IBM Corp. ` + string(rune(currentYear/ 2026+48)) + string(rune((currentYear/100)%10+48)) + string(rune((currentYear/10)%10+48)) + string(rune(currentYear%10+48)) + `
package main
`,
			targetHolder:      "IBM Corp.",
			configYear:        currentYear,
...
`,
		...
		{
			name: "Regular file",
			content: `// Copyright IBM Corp. 2026
package main
...
func TestParseCopyrightLine_InlineComment(t *testing.T) {
	line := "var x := 1 // Copyright IBM Corp. 2026"
...
func TestUpdateCopyrightHeader_InlineCommentPreserved(t *testing.T) {
	...
	initial := "var x := 1 // Copyright IBM Corp. 2026\n"
	...
	content, err := os.ReadFile(testFile)
	require.NoError(t, err)
	expected := "var x := 1 // Copyright IBM Corp. 2026, " + strconv.Itoa(currentYear) + "\n"
...
func TestUpdateCopyrightHeader_IgnoreYear1(t *testing.T) {
	...
	initial := `// Copyright IBM Corp. 2026
package main
`
	err := os.WriteFile(testFile, []byte(initial), 0644)
	require.NoError(t, err)
...
`
	...
	content, err := os.ReadFile(testFile)
	require.NoError(t, err)
	expected := fmt.Sprintf("// Copyright IBM Corp. 2026, %d\npackage main\n", currentYear)
	assert.Equal(t, expected, string(content))
}
